...
import 'package:upgrade_manager/models/appcast.dart';
import 'package:upgrade_manager/models/appcast_item.dart';
import 'package:upgrade_manager/models/upgrade_status.dart';
import 'package:upgrade_manager/utils/current_version_manager.dart';
...
class UpgradeManager {
  ...
  void init({
    ...
    CurrentVersionManager.load(currentAppVersion, crashIfNoLegalConfigFile,
        (version) {
  ...
  Future<void> checkForUpdates() async {
    ...
    try {
      if (response.statusCode != 200) {
        ...
        Logger.printError(
            "[UpgradeManager] Download Appcast from $_url error.");
        return;
      }
      ...
      if (best != null && state.current?.version != best.version) {
        state.updateLatestVersion(version: best);
        initInstallers();
        nextInstaller();
      } else {
        Logger.printMessage("[UpgradeManager] No new version.");
